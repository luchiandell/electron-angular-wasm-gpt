...
(function () {
  ...
  var loadPackage = function (metadata) {
    ...
    var PACKAGE_NAME = "../../bin/gpt-2.data";
    var REMOTE_PACKAGE_BASE = "gpt-2.data";
    if (
    ...
    function runWithFS() {
      ...
      function processPackageData(arrayBuffer) {
        ...
        Module["removeRunDependency"]("datafile_../../bin/gpt-2.data");
      }
      Module["addRunDependency"]("datafile_../../bin/gpt-2.data");
  ...
  loadPackage({
    files: [
      {
        filename: "/models/Cerebras-GPT-1.3B-Alpaca-SP/ggml-model.bin",
        start: 0,
        end: 903514810,
      },
    ],
    remote_package_size: 903514810,
    package_uuid:
      "sha256-bf7684712a1f8bb301dd35c2de1477b8c189dfb8bcfd3a656b8709d958b39358",
  });
})();
...
var PThread = {
  ...
  loadWasmModuleToWorker: (worker) =>
    new Promise((onFinishedLoading) => {
      ...
      worker.onerror = (e) => {
        ...
        if (worker.pthread_ptr) {
          message =
            "Pthread " + ptrToString(worker.pthread_ptr) + " sent an error!";
        }
  ...
  allocateUnusedWorker: function () {
    var worker;
    var pthreadMainJs = locateFile("gpt-2.worker.js");
...
function readEmAsmArgs(sigPtr, buf) {
  ...
  while ((ch = HEAPU8[sigPtr++ >>> 0])) {
    ...
    readEmAsmArgsArray.push(
      ch == 105 ? HEAP32[buf >>> 0] : HEAPF64[buf++ >>> 1]
    );
    ++buf;
  }
...
function _emscripten_receive_on_main_thread_js(index, numCallArgs, args) {
  ...
  var b = args >> 3;
  for (var i = 0; i < numCallArgs; i++) {
    emscripten_receive_on_main_thread_js_callArgs[i] = HEAPF64[(b + i) >>> 0];
  }
...
function _environ_get(__environ, environ_buf) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(8, 1, __environ, environ_buf);
  var bufSize = 0;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(9, 1, penviron_count, penviron_buf_size);
...
function _fd_write(fd, iov, iovcnt, pnum) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(13, 1, fd, iov, iovcnt, pnum);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
var __emscripten_proxy_main = (Module["__emscripten_proxy_main"] =
  createExportWrapper("_emscripten_proxy_main"));
...
var dynCall_iiiifff = (Module["dynCall_iiiifff"] =
  createExportWrapper("dynCall_iiiifff"));
